...
function practicePage(){if(teacher)return title('学生练习记录','查看当前所选学生的作答情况。')+practiceDetails(pupil())+variantPracticeDetails(pupil());return title('练习与错题','先完成本课练习，再挑战 AI 拔高题。')+`<section class="panel practice-course-picker"><label for="practice-course">选择练习课程</label><select id="practice-course">${LESSONS.map((l,i)=>`<option value="${i}" ${i===practiceCourse?'selected':''}>第${i+1}课 · ${esc(l.title)}</option>`).join('')}</select></section>`+studentPracticePanel(practiceCourse);}
