...
function teachingMethod(){const l=LESSONS[lessonId];return courseVideoPanel()+teacherPrivate(`<section class="panel"><h2>视频与母题怎样衔接</h2><p>${lessonId===0?'本页依次对应视频的两道例题：水瓶茶杯的直接消去、橡皮铅笔的配套。每道例题有独立题卡、答案与步骤。':'本页母题与视频的例题逐题对应；看完一题，就回到对应题卡整理方法。'}</p><p>在讲解章节切换前暂停，让学生说“接下来想求什么，为什么”；最后的“换你来讲”安排在小老师环节。变式中的变化已用红字与文字说明标出。</p><details><summary>回看母题与分步讲法</summary>${motherProblem(l)}${l.steps.map(s=>`<h3>${esc(s[0])}</h3><p>${esc(s[1])}</p><p>${esc(s[2])}</p>`).join('')}</details>${variantsView(l)}</section>`);}
function teachingView(){
 ...
 const content=()=>s===0?teachingPreparation()+teacherPrivate(personalReviewPanel()):s===1?motherProblem(l)+`<section class="panel"><h2>先尝试，不急着听答案</h2><p>圈出已知条件和所求，在草稿纸上画图或列式。</p><p>说一说：我知道了什么？还缺什么？我打算从哪里开始？</p></section>`:s===2?teachingMethod():s===3?`<section class="panel"><h2>先预测，再验证</h2>${teacherPrivate(`<p>${esc(l.detail.labTask)}</p>`)}<p>${lessonId===3?'先分一分找因数，再分家族辨质数，最后独立闯关。建议用8分钟操作、7分钟分类与闯关，余下5分钟让学生说依据。':'先按原题观察；说清关键一步以后再调整参数做变式。'}普通课在本环节前休息5分钟（计入本环节25分钟）。</p></section>`+(lessonId===3?factorGameView():labView(lessonId)):s===4?talkView():s===5?`<section class="panel"><h2>独立完成这3道题</h2><p>学生在自己的账号中进入第${lessonId+1}课“独立练习”，用草稿纸作答后提交。测评课先做第1题，其余按速度安排。</p>${l.practice.map((q,j)=>practiceQuestion(q,lessonId,j)).join('')}</section>`:lessonRecordPanel();
