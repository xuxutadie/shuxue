...
function shell(){
 ...
 const nav=teacher?[['home','⌂','班级工作台'],['courses','▤','课程与教案'],['exams','✎','学生测评'],['homework','▧','变式作业'],['ai-practice','✦','AI 引导练习'],['report','▥','学生成长档案'],['teacher','⚙','班级与资源']]:[['home','⌂','我的探索基地'],['courses','⚑','六周探险地图'],['games','◇','思维游乐场'],['practice','✓','练习与错题'],['report','▥','我的成长足迹']];
...
async function render(options={}){
 ...
 try{
  ...
  else if(teacher&&!pupil()&&!['home','teacher','courses','lesson','games','game','ai-practice','ai-settings','homework'].includes(route)){main.innerHTML=title('先创建一位学生','创建账号后即可查看课程进度、布置测评并记录课堂表现。')+button('创建学生账号','create-student');}
