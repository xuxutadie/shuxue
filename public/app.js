...
async function api(url,options={}){
 ...
 const practiceCheck=method==='POST'&&/^\/api\/students\/[^/]+\/(?:practice|variants)\/\d+\/\d+$/.test(url);
...
function shell(){
 const logged=!!user;document.getElementById('logout').hidden=!logged;document.getElementById('account-button').hidden=!logged||!!previewStudentId;document.getElementById('refresh-data').hidden=!logged;
 ...
 const selector=document.getElementById('student');selector.hidden=!logged||user.role!=='teacher'||!overview.students.length;
 ...
 const nav=teacher?[['home','⌂','班级工作台'],['courses','▤','课程与教案'],['exams','✎','学生测评'],['ai-practice','✦','AI 引导练习'],['report','▥','学生成长档案'],['teacher','⚙','班级与资源']]:[['home','⌂','我的探索基地'],['courses','⚑','六周探险地图'],['exams','✎','测评挑战'],['games','◇','思维游乐场'],['practice','✓','练习与错题'],['report','▥','我的成长足迹']];
...
function dashboard(){const ps=overview.students,a=ps.filter(p=>p.exams.A),b=ps.filter(p=>p.exams.B),needs=ps.filter(p=>reminders(p).length);return title(`你好，${esc(user.name)}。`,'先看看孩子们的进展，把时间留给最需要帮助的地方。',button('创建学生账号','create-student')+'<button type="button" class="secondary" data-scroll="exam-analysis">查看班级错题</button>')+`<section class="hero teacher-banner"><div class="hero-content"><span class="eyebrow">TEACHER’S CORNER</span><h2>每个孩子，都有自己的成长路线。</h2><p>${ps.length?`目前有 ${ps.length} 位学生，${needs.length} 位有待跟进任务。点击姓名查看答题、讲课和练习记录。`:'先创建学生账号，再为班级布置起点测评。学生登录后即可开始自己的学习旅程。'}</p><div class="controls">${button('布置班级测评','assign-class','secondary')}${button('班级与资源','teacher-open','quiet')}</div></div><div class="hero-art">${mascot}</div></section>`+stats([['我的学生',ps.length,'位'],['已完成前测',a.length,'位'],['已完成后测',b.length,'位'],['待跟进',needs.length,'位']])+`${examAnalysisPanel()}<section class="panel"><div class="section-head"><h2>学生学习一览</h2>${button('导出学习记录','export','quiet')}</div><div class="toolbar"><label class="sr-only" for="student-search">搜索学生</label><input id="student-search" placeholder="搜索学生姓名或账号"><label class="sr-only" for="class-filter">筛选班级</label><select id="class-filter"><option value="">全部班级</option>${overview.classes.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}</select></div><div id="student-table">${studentTable(ps)}</div></section>`;}
...
function examsPage(){const p=pupil();return (teacher?'<p class="notice">前测A卷已换为同难度新题。教师答案与打印学生卷对应当前新卷；已开始或已提交的测评保留原题，请从该生答题报告查看原卷解析。</p>':'')+title(teacher?`${esc(p.name)}的测评`:'测评挑战','两次独立作答，看看六周里发生了什么变化。')+`<div class="notice">每卷20道填空题，每题6分，总分120分。45分钟。可以打草稿，不使用计算器、AI或老师提示。</div><div class="grid-two">${['A','B'].map(k=>{const e=p.exams[k],d=p.drafts[k];return `<section class="panel"><span class="tag">${k==='A'?'认识起点':'看见成长'}</span><h2>${k==='A'?'前测 A 卷':'后测 B 卷'}</h2><p>20题 · 120分 · 45分钟</p>${e?`<p class="game-number">${e.score} <small>/ 120分</small></p><p class="tiny">${new Date(e.date).toLocaleString('zh-CN')}</p>${teacher||e.released?button('查看答题与解析','exam-review','',`data-kind="${k}"`):'<p class="muted">已交卷。错题和解析将在老师开放讲评后显示。</p>'}`:teacher?`<p>${d?'学生正在作答':p.assignments[k]?'已布置，等待学生开始':'尚未布置'}</p>${!d?button(p.assignments[k]?'撤回未开始任务':'布置给这位学生','assign-student','secondary',`data-kind="${k}" data-enabled="${!p.assignments[k]}"`):''}`:p.assignments[k]||d?button(d?'继续作答 →':'查看考试说明 →','exam-open','',`data-kind="${k}"`):'<div class="empty">等老师布置后，就可以开始啦。</div>'}${teacher?`<div class="controls">${button('教师答案','exam-key','quiet',`data-kind="${k}"`)}${button('打印学生卷','print-exam','quiet',`data-kind="${k}"`)}</div>`:''}</section>`;}).join('')}</div>${historyPanel()}`;}
...
function syncStudentPreviewShell(){
 ...
 toggle.textContent=previewStudentId?'返回教师模式':'预览学生模式';
 banner.hidden=!previewStudentId;
 if(previewStudentId){
  const p=pupil();
  document.getElementById('identity').textContent=`教师预览 · ${p?.name||'正在载入'}`;
  banner.innerHTML=`<strong>正在预览：${esc(p?.name||'学生')}的学生页面</strong><p>可看视频、操作动画和试做固定练习。不会保存讲课准备、修改成绩、启动考试或消耗学生 AI 额度。${p?.accountDisabled?'该账号已停用，学生本人目前无法登录。':''}</p>`;
 }
}
...
async function render(options={}){
 ...
 try{
  ...
  if(route==='ai-practice'&&!teacher){practiceTab='ai';location.hash='practice';return;}
  ...
  if(route==='account')main.innerHTML=previewStudentId?title('学生账号设置','预览不修改学生密码。需要重置时，请返回教师端操作。'):passwordView();
