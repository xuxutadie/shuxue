...
const C={ink:'#243047',cream:'#FFF9E9',blue:'#75C9FF',yellow:'#FFDB58',purple:'#C6B1FF',mint:'#A8E4CA',coral:'#FF9B77'};
...
function readScene(contest=false){
 ...
 s+=group('note',text(65,647,contest?'两种数量都变了：还不能直接抵消。':'总价：整次购买的钱　　单价：一个商品的价格',34));
...
const renderers={read:()=>readScene(),pair:pairScene,share:shareScene,back:backScene,contest:()=>readScene(true),trap:trapScene,multiple:multipleScene,copya:()=>copyScene('a'),copyb:()=>copyScene('b'),eliminate:eliminateScene,verify:verifyScene,challenge:challengeScene};
...
for(scene of scenes){
 ...
 scene.duration=+(cursor+.7).toFixed(3);duration+=scene.duration;
 code='';events=[];
 ...
 const html=`<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"></head><body><template><style>${css}</style><div id="${scene.id}" data-composition-id="${scene.id}" data-width="1920" data-height="1080" data-duration="${scene.duration}"><div class="scene"><div class="head"><span class="tag">${scene.tag}</span><span>思维实验室 · 把数学讲明白</span><span>${scenes.map((_,j)=>`<i class="chapter-dot ${j<=scenes.indexOf(scene)?'active':''}"></i>`).join('')}</span></div><h1 id="${id('heading')}">${scene.title}</h1><svg class="board" viewBox="0 0 1760 680" role="img" aria-label="${scene.title}">${art}</svg><div class="source">${source}</div>${subs}<div class="bar" id="${id('progress')}"></div></div></div><script>{const tl=gsap.timeline({paused:true});${code}window.__timelines['${scene.id}']=tl;}</script></template></body></html>`;
...
fs.writeFileSync(path.join(root,'第一课-原题精讲-字幕.srt'),captions.map((c,i)=>`${i+1}\n${stamp(c.start)} --> ${stamp(c.end)}\n${c.text}\n`).join('\n'));
fs.writeFileSync(path.join(root,'SCRIPT.md'),'# 第一课原题精讲\n\n'+scenes.map(s=>`## ${s.title}\n\n${s.lines.map(l=>l.text).join('\n\n')}`).join('\n\n'));
