...
function sandbox(isTeacher=true){
 ...
 for(const file of ['course-videos.js','figures.js','lesson-lab.js','games.js','teacher-flow.js','views.js','app.js'])vm.runInContext(fs.readFileSync('public/'+file,'utf8').replace(/startSession\(\);\s*$/,''),context);
